function ghcd --description "Shorthand for cd-ing to a repo from GitHub to the standardized place" --argument repo
	...
	if string match -q -- "*/*" $repo
		...
		echo "Was expecting to find '$HOME/Desktop/GitHub/$repo', but does not exist."
	else
		...
		for r in (find -P $HOME/Desktop/GitHub -type d -maxdepth 2 -mindepth 2)
			set bn (basename $r)
		...
		echo "Was expecting to find a repo under $HOME/Desktop/GitHub with a basename of $repo, but does not exist."
	end
	return 1
end
